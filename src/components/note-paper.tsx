...
export const NotePaper: React.FC<NotePaperProps> = ({ onAddNote }) => {
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <div className="group z-10 -rotate-6 hover:scale-110 transform transition-all duration-300 ease-in-out hover:-translate-y-10 group-hover:rotate-12">
          ...
          <div className="relative w-32 h-32">
            ...
            <div
              className={`relative w-full h-full ${color} shadow-md rounded-sm transform transition-all duration-300 ease-in-out group-hover:rotate-8 group-hover:translate-x-2 group-hover:-translate-y-2 group-hover:shadow-2xl`}
            >
              {/* Paper texture */}
              ...
              <div className="absolute inset-0 p-4 opacity-20">
                ...
                >
                  <path
                    d="M10 20 Q 20 25, 30 20 T 50 20 T 70 20 T 90 20"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="1"
                  />
                  <path
                    d="M10 40 Q 25 45, 40 40 T 70 40 T 90 40"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="1"
                  />
            ...
            <div className="absolute -top-3 -right-3 w-8 h-12 transform rotate-12 z-10 transition-all duration-300 ease-in-out group-hover:translate-x-1 group-hover:-translate-y-1">
              ...
              >
                <path
                  d="M9 6C9 3.23858 11.2386 1 14 1H18C20.7614 1 23 3.23858 23 6V38C23 41.3137 20.3137 44 17 44H15C11.6863 44 9 41.3137 9 38V6Z"
      ...
      <PopoverContent
        className="w-auto p-0 m-8 rounded-2xl bg-white shadow-lg"
        sideOffset={5}
      >
        <div className="grid grid-cols-5 gap-2 p-2">
          {colorOptions.map((option) => (
            <button
              key={option.value}
              className={`w-10 h-10 rounded-full border-2 border-stone-200 ${option.value} hover:brightness-110 hover:scale-105 transition-all duration-200 relative flex items-center justify-center`}
              onClick={() => setColor(option.value)}
              aria-label={`Set note color to ${option.name}`}
            >
              {color === option.value && (
                <Check className="w-5 h-5 text-stone-700" strokeWidth={3} />
              )}
            </button>
          ))}
        </div>
